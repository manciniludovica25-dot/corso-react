...
async function aggiornaUIOrdine(id) {
...
function mostraFormModifica(id) {
    ...
    const categoriaAttuale = menuRistorante.find((p) => p.nome === piattoAttuale)?.categoria || "";
    ...
    ${ordine.cliente.type === "persona"
      ...
      <label>Cognome: <input type="text" id="cognome-mod-${id}" value="${ordine.cliente.cognome}" /></label><br>
    `
        : `
  ...
  `;
    ...
    const selectCategoria = document.getElementById(`categoria-mod-${id}`);
    selectCategoria.addEventListener("change", () => {
        const selectPiatti = document.getElementById(`piatto-mod-${id}`);
...
async function confermaModifica(id) {
    ...
    if (!ordine)
        return;
    const nomeInput = document.getElementById(`nome-mod-${id}`);
    const piattoInput = document.getElementById(`piatto-mod-${id}`);
    let clienteModificato;
    if (ordine.cliente.type === "persona") {
        const cognomeInput = document.getElementById(`cognome-mod-${id}`);
        clienteModificato = {
            ...ordine.cliente,
...
function annullaCancellazione(id) {
...
async function confermaCancellazione(id) {
...
async function inviaOrdine() {
  ...
  `;
    ...
    elaboraOrdine(nuovoOrdine.id, 0, cliente).then((risultato) => {
...
window.mostraFormModifica = mostraFormModifica;
window.confermaModifica = confermaModifica;
window.annullaModifica = annullaModifica;
window.mostraFormCancellazione = mostraFormCancellazione;
window.confermaCancellazione = confermaCancellazione;
window.annullaCancellazione = annullaCancellazione;
// inizializza il menu nel select al caricamento della pagina
function filtraPiattiPerCategoria(categoria) {
    if (!categoria)
        return [];
